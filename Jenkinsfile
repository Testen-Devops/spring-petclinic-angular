                ...
                script {
                    withCredentials([usernamePassword(credentialsId: 'remote-server-credentials', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                        ...
                        try {
                            ...
                            sshCommand remote: remote, command: 'docker run --detach --rm --volume /etc/letsencrypt/live/server.ninopeters.de/:/etc/letsencrypt/live/server.ninopeters.de/ --publish 80:80 --publish 443:443 --name spring-petclinic-angular npetersdev/spring-petclinic-angular:latest'
                        }
                    }
                }
            }
